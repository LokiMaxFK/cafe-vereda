...
import { ArrowRight, BarChart3, BookOpen, Boxes, ChefHat, Clock3, Coffee, LayoutGrid, ShoppingBag, Users, WalletCards } from "lucide-react";
...
interface QuickAction { href: string; label: string; description: string; icon: ReactNode; managerOnly?: boolean; }

const quickActions: QuickAction[] = [
  { href: "/salon", label: "Salón", description: "Abrir una mesa o iniciar pedido para llevar", icon: <LayoutGrid size={20} /> },
  { href: "/preparacion", label: "Preparación", description: "Ver la cola de comandas activas", icon: <ChefHat size={20} /> },
  { href: "/pedidos", label: "Pedidos", description: "Historial completo de la operación", icon: <Coffee size={20} /> },
  { href: "/caja", label: "Caja", description: "Cortes, retiros y efectivo esperado", icon: <WalletCards size={20} />, managerOnly: true },
  { href: "/reportes", label: "Reportes", description: "Ventas, ticket promedio y más vendidos", icon: <BarChart3 size={20} />, managerOnly: true },
  { href: "/insumos", label: "Insumos", description: "Existencias y movimientos de kardex", icon: <Boxes size={20} />, managerOnly: true },
  { href: "/catalogo", label: "Catálogo", description: "Productos, precios y disponibilidad", icon: <BookOpen size={20} />, managerOnly: true },
  { href: "/personal", label: "Personal", description: "Accesos y turnos del equipo", icon: <Users size={20} />, managerOnly: true }
];
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {actions.map((action) => (
          <button
            ...
            className="group flex flex-col items-start gap-3 rounded-2xl border border-outline-variant/35 bg-surface-container-lowest p-5 text-left shadow-panel transition hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-panel-hover"
          >
            <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-primary-fixed text-primary">{action.icon}</span>
            <div className="min-w-0">
              <p className="flex items-center gap-1 font-bold text-on-surface">{action.label}<ArrowRight size={15} className="text-outline transition group-hover:translate-x-0.5 group-hover:text-primary" /></p>
              <p className="mt-1 text-sm leading-5 text-on-surface-variant">{action.description}</p>
            </div>
          </button>
        ))}
      </div>
