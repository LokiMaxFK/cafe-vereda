...
import { nextCopyNumber, peekCopyNumber, resetCopyNumber } from "../lib/printCopies";
...
/**
 * Texto del panel central cuando la cuenta ya no admite productos. Antes era uno solo y decía
 * "lista para cobrar · usa el botón «Cobrar»" incluso en una venta ya cobrada, cancelada o
 * revertida, donde ese botón ni siquiera existe.
 */
function closedStateCopy(status: Order["status"]): { title: string; description: string } {
  if (status === "closed") return { title: "Venta cobrada", description: "La cuenta ya está pagada y cerrada. Usa «Reimprimir» todas las veces que el cliente necesite otra copia del ticket." };
  if (status === "cancelled") return { title: "Cuenta cancelada", description: "El registro se conserva para auditoría, pero ya no se puede modificar ni cobrar." };
  if (status === "reversed") return { title: "Venta revertida", description: "El cobro se anuló y quedó registrado en Reportes. La cuenta ya no se puede modificar." };
  return { title: "Orden finalizada, lista para cobrar", description: "Ya no se pueden agregar productos. Usa el botón «Cobrar» para registrar el pago." };
}
...
export function SalePage() {
  ...
  // Hay algo ya comandado que se puede reenviar a la barra: la comanda se puede repetir sin límite
  // (impresora atascada, la barra la extravió, se pidió otra copia…), siempre marcada como COPIA N.
  const hasCommandedItems = order.items.some((item) => item.dispatchBatchId);
  ...
  // Con la cuenta dividida y algún cobro hecho, cambiar el descuento movería lo que ya pagaron:
  // el total de cada persona se deriva del de la cuenta, no se guarda.
  const splitFrozen = hasSubaccountPayments(order);
  const discountable = (editable || order.status === "served") && !splitFrozen;
  // Las subcuentas reparten el total de la cuenta, no lo que queda por cobrar, así que dividir
  // después de un pago dejaría a la última persona debiendo un saldo que ya no existe y la cuenta
  // sin poder cerrarse. Con un cobro hecho, la cuenta se termina entera.
  const splittable = order.payments.length === 0;
  ...
  async function dispatch() {
    ...
    if (batchId) {
      // La comanda original no lleva número de copia, así que la cuenta arranca de cero otra vez.
      resetCopyNumber(activeOrder.id);
  ...
  async function finish() {
    ...
    try {
      await printTicket(refreshed);
      setMessage("Venta cerrada. Usa «Reimprimir» para repetir el ticket las veces que haga falta.");
    } catch (reason) { setMessage(`Venta cerrada, pero no se pudo imprimir el ticket: ${printErrorMessage(reason)}`); }
    // No se sale de la cuenta al cerrar: el cajero se queda aquí para reimprimir el ticket las veces
    // que el cliente pida. Vuelve al salón con la flecha del encabezado cuando termina.
    setCheckoutOpen(false);
  }
  async function reprintTicket() {
    setMessage("");
    try {
      await printTicket(activeOrder);
      setMessage("Se abrió el diálogo de impresión del ticket. Puedes repetirlo las veces que haga falta.");
    } catch (reason) { setMessage(`No se pudo imprimir el ticket: ${printErrorMessage(reason)}`); }
  }
  async function reprintCommand() {
    const items = activeOrder.items.filter((item) => item.dispatchBatchId);
    if (!items.length) return;
    setMessage("");
    // El número de copia se reserva antes de imprimir para que dos reimpresiones seguidas no
    // salgan las dos como COPIA 1 y la barra las confunda con comandas distintas.
    const copy = nextCopyNumber(activeOrder.id);
    try {
      await printCommand(activeOrder, items, copy);
      setMessage(`Se abrió el diálogo de impresión de la COPIA ${copy} de la comanda.`);
    } catch (reason) { setMessage(`No se pudo reimprimir la comanda: ${printErrorMessage(reason)}`); }
  }
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      <header className="sticky top-0 z-30 flex min-h-16 items-center justify-between gap-2 border-b border-outline-variant/30 bg-background/95 px-4 py-2 backdrop-blur sm:gap-3 sm:px-6">
        <div className="flex min-w-0 items-center gap-2 sm:gap-3"><Button size="icon" variant="ghost" className="shrink-0" onClick={() => navigate("/salon")} aria-label="Volver al salón"><ArrowLeft size={20} /></Button><div className="min-w-0"><div className="flex items-center gap-2"><h1 className="truncate text-base font-bold sm:text-lg">{order.type === "table" ? `Mesa ${order.tableId?.replace("t", "")}` : order.customerName || "Para llevar"}</h1><OrderStatusBadge status={order.status} className="hidden shrink-0 sm:inline-flex" /></div><div className="flex items-center gap-1.5"><OrderStatusBadge status={order.status} className="shrink-0 sm:hidden" /><p className="truncate text-xs text-on-surface-variant">Orden #{order.folio} · {new Date(order.openedAt).toLocaleTimeString("es-MX", { hour: "2-digit", minute: "2-digit" })}</p></div></div></div>
        <div className="relative flex shrink-0 items-center gap-1.5 sm:gap-2">{discountable && <Button className="hidden sm:inline-flex" onClick={() => openDiscount(false)} disabled={session?.role !== "manager"}>Descuento</Button>}{order.status === "closed" ? <Button variant="primary" onClick={() => void reprintTicket()}><Printer size={18} /> Reimprimir</Button> : isChargeable(order) ? <Button variant="primary" onClick={() => (isSplit(order) ? setSplitOpen(true) : setCheckoutOpen(true))} disabled={!order.items.length}>Cobrar <ChevronRight size={18} /></Button> : isFinalizable(order) ? <Button variant="success" onClick={() => void finalize()} disabled={!order.items.length || pendingItems.length > 0}><ClipboardCheck size={18} /> Finalizar<span className="hidden sm:inline">&nbsp;orden</span></Button> : null}<Button size="icon" variant="ghost" aria-label="Más acciones" onClick={() => setActionsOpen((value) => !value)}><MoreVertical size={19} /></Button>{actionsOpen && <div className="absolute right-0 top-12 z-40 w-56 rounded-xl border border-outline-variant/40 bg-surface-container-lowest p-2 shadow-2xl">{cancellableStatuses.includes(order.status) && <button className="flex min-h-11 w-full items-center gap-2 rounded-lg px-3 text-sm font-semibold text-error hover:bg-error-container" onClick={() => { setOrderAction("cancel"); setActionsOpen(false); }}><Trash2 size={17} /> Cancelar cuenta</button>}{order.status === "closed" && session?.role === "manager" && <button className="flex min-h-11 w-full items-center gap-2 rounded-lg px-3 text-sm font-semibold text-error hover:bg-error-container" onClick={() => { setOrderAction("reverse"); setActionsOpen(false); }}><RotateCcw size={17} /> Revertir venta</button>}{hasCommandedItems && <button className="flex min-h-11 w-full items-center gap-2 rounded-lg px-3 text-sm font-semibold hover:bg-surface-container-low" onClick={() => void reprintCommand()}><Printer size={17} /> Reenviar comanda a cocina (COPIA {peekCopyNumber(order.id)})</button>}{order.items.length > 0 && <button className="flex min-h-11 w-full items-center gap-2 rounded-lg px-3 text-sm font-semibold hover:bg-surface-container-low" onClick={() => void reprintTicket()}><Printer size={17} /> Imprimir ticket</button>}</div>}</div>
      </header>
      ...
      {checkoutOpen && <Modal title={`Cobrar orden #${order.folio}`} description="Puedes registrar varios pagos para dividir la cuenta." onClose={() => setCheckoutOpen(false)} width="max-w-xl"><div className="rounded-2xl bg-primary p-5 text-on-primary"><p className="text-xs font-bold uppercase tracking-wider text-on-primary/65">Saldo pendiente</p><p className="mt-1 text-4xl font-bold">{mxn.format(balance)}</p><div className="mt-3 flex justify-between text-sm text-on-primary/70"><span>Total {mxn.format(total)}</span><span>Pagado {mxn.format(paid)}</span></div></div>{balance > 0 && splittable && <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-outline-variant/40 p-3 text-sm"><span className="min-w-0 text-on-surface-variant">¿Pagan por separado? Divide la cuenta y cobra a cada quien lo suyo.</span><Button size="sm" variant="primary" onClick={() => { setCheckoutOpen(false); setSplitOpen(true); }}><Users size={16} /> Cuentas separadas</Button></div>}{session?.role === "manager" && balance > 0 && <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-outline-variant/40 p-3 text-sm"><span className="min-w-0 text-on-surface-variant">{order.discount > 0 ? <>Descuento <strong className="text-tertiary">-{mxn.format(order.discount)}</strong>{order.discountReason ? ` · ${order.discountReason}` : ""}</> : "Sin descuento aplicado"}</span><Button size="sm" onClick={() => openDiscount(true)}>{order.discount > 0 ? "Editar descuento" : "Aplicar descuento"}</Button></div>}{order.payments.length > 0 && <div className="my-4 space-y-2">{order.payments.map((payment) => <div key={payment.id} className="flex items-center justify-between rounded-xl bg-surface-container-low p-3 text-sm"><span className="font-semibold">{paymentMethodLabel[payment.method] ?? payment.method}</span><span>{mxn.format(payment.amount)}{payment.tip ? ` + ${mxn.format(payment.tip)} propina` : ""}{paymentChange(payment) > 0 ? <strong className="ml-2 text-tertiary">Cambio {mxn.format(paymentChange(payment))}</strong> : null}</span></div>)}</div>}{balance > 0 ? <div className="mt-5 space-y-4"><div className="grid grid-cols-3 gap-2">{paymentOptions.map((option) => { const Icon = option.icon; return <button key={option.value} onClick={() => setMethod(option.value)} className={`flex min-h-20 flex-col items-center justify-center gap-2 rounded-xl border text-xs font-bold ${method === option.value ? "border-primary bg-primary-fixed text-primary" : "border-outline-variant/40"}`}><Icon size={20} />{option.label}</button>; })}</div><div className="grid gap-3 sm:grid-cols-2"><label className="text-sm font-semibold">Importe<TextField type="number" min="1" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder={balance.toFixed(2)} />{Number(amount) > balance && <span className="mt-1 block text-xs font-medium text-tertiary">Cambio: {mxn.format(Number(amount) - balance)}</span>}</label><label className="text-sm font-semibold">Propina aparte<TextField type="number" min="0" value={tip} onChange={(event) => setTip(event.target.value)} /></label></div><div className="flex gap-2"><Button className="flex-1" onClick={() => setAmount(balance.toFixed(2))}>Saldo exacto</Button><Button className="flex-1" variant="primary" disabled={!Number(amount)} onClick={() => void registerPayment()}>Registrar pago</Button></div></div> : <div className="mt-5"><InlineAlert tone="success"><span className="flex items-center gap-2"><Check size={18} /> La cuenta está cubierta. Ya puedes cerrarla.</span></InlineAlert><Button variant="primary" size="lg" className="mt-4 w-full" onClick={() => void finish()}><Printer size={18} /> Cerrar e imprimir ticket</Button></div>}</Modal>}
      {splitOpen && <SplitBillModal order={order} onClose={() => setSplitOpen(false)} onSettled={() => { setSplitOpen(false); navigate("/salon"); }} />}
    </div>
  );
}
