...
export function ProductPicker({ onSelect }: { onSelect: (selection: ProductPickerSelection) => void }) {
  ...
  const visibleProducts = useMemo(() => products.filter((product) => product.available && (search ? `${product.name} ${product.description ?? ""}`.toLowerCase().includes(search.toLowerCase()) : product.categoryId === categoryId)), [products, categoryId, search]);
  const eligibleExtras = useMemo(() => variantProduct ? extras.filter((extra) => extra.active && variantProduct.modifierIds?.includes(extra.id)) : [], [extras, variantProduct]);

  function selectProduct(product: Product) {
    if (product.variants?.length || product.modifierIds?.length) { setSelectedVariant(product.variants?.[0]?.id); setSelectedModifiers([]); setItemNotes(""); setVariantProduct(product); }
