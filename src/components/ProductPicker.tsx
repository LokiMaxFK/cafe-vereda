...
import { hasPriceChoices, productDisplayPrice } from "../domain/catalog";
...
export function ProductPicker({ onSelect }: { onSelect: (selection: ProductPickerSelection) => void }) {
  ...
  return (
    <>
      <aside className="border-b border-outline-variant/30 bg-surface-container-low px-3 py-3 lg:col-span-2 xl:col-span-1 xl:border-b-0 xl:border-r xl:py-5">
      ...
      <section className="min-w-0 p-4 sm:p-6">
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3 2xl:grid-cols-4">{visibleProducts.map((product) => <button key={product.id} onClick={() => selectProduct(product)} className="group flex min-h-36 flex-col justify-between rounded-2xl border border-outline-variant/35 bg-surface-container-lowest p-4 text-left shadow-panel transition hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-panel-hover"><div>{product.imageUrl ? <img src={product.imageUrl} alt={product.name} className="mb-3 h-16 w-16 rounded-xl object-cover" /> : <div className="mb-3 flex h-9 w-9 items-center justify-center rounded-xl bg-primary-fixed text-primary"><Coffee size={18} /></div>}<div className="flex items-center gap-1.5"><h3 className="font-bold leading-5 text-on-surface">{product.name}</h3>{product.seasonal && <Sparkles size={14} className="shrink-0 text-primary" />}</div>{product.description && <p className="mt-1 line-clamp-2 text-xs leading-5 text-on-surface-variant">{product.description}</p>}</div><div className="mt-3 flex items-end justify-between gap-2"><span className="text-base font-bold text-primary">{mxn.format(productDisplayPrice(product))}{hasPriceChoices(product) ? "+" : ""}</span></div></button>)}</div>
      </section>
