...
- **Suite de 180 → 201 pruebas en verde**, lint y build limpios. Sólo creció.
- **Después de la entrega** se corrigió **F08-05 (Alta)**: cancelar una cuenta desde el listado de
  Pedidos o desde la vista previa del Salón no avisaba a la barra. La regla vive ahora en un único
  sitio (`barItemsForCancellation` + `cancelOrderNotifyingBar`), con 5 pruebas y los tres caminos
  verificados en el navegador. **Desplegado en `cafevereda.ithakon.tech`.**

### Qué falta

**Nada de la revisión.** Lo que queda son **decisiones del responsable**, no trabajo de verificación:

1. **6 hallazgos abiertos**, todos con el arreglo ya analizado en §0.7 — uno de severidad **Alta**:
   **F14-02** (desactivar un acceso no lo revoca). Los demás son Media: F11-02, F14-03, F14-04,
   F14-05 y F16-04. *(**F08-05**, el otro Alta, se corrigió y verificó el 20/08 tras la entrega:
   los tres caminos de cancelación avisan ya a la barra.)*
...
| F08 | Cancelación y reversión | `/venta/:id`, `/pedidos`, `/salon` | A + B | ⚠️ Completada (3 correcciones, la última **F08-05**; F08-02 documentada) | [F08](pdf/F08-cancelacion-y-reversion.pdf) |
...
| F17 | Cuentas separadas | `/venta/:id` (cobro) | A + B | ✅ Módulo nuevo, verificado en navegador y contra Postgres real (1 corrección: **F17-01**, auditoría que habría fallado en cada escritura) | [F17](fichas/F17-cuentas-separadas.html) |
...
| F08-05 | **Alta** | **La corrección de F08-03 sólo cubre una de las tres formas de cancelar una cuenta: desde las otras dos, la barra sigue sin enterarse.** F08-03 añadió la impresión de la comanda de cancelación en `SalePage.tsx:140-142`, dentro de la pantalla de la venta (`/venta/:id`), y ahí funciona. Pero cancelar una cuenta completa **también** se puede hacer desde el listado de **Pedidos** (`OrdersPage.tsx:5`) y desde la **vista previa del Salón** (`OrderPreviewModal.tsx:77`), y las dos usan `CancelOrderModal`, que llama a `cancelOrder(order.id, reason)` y **no llama a `printCommand` en ningún momento** (`CancelOrderModal.tsx:13-22`). Verificado en producción el 20/08 con el interceptor de impresión de §0.9 activo: al cancelar desde `/pedidos` la orden #1085 —que tenía su renglón ya **despachado a la barra**— la orden quedó `cancelled` con su motivo en el servidor y **no se generó ninguna comanda de cancelación** (`__qaPrintDocs` vacío, con el interceptor demostradamente funcionando: había capturado comandas y tickets minutos antes en la misma sesión). **Consecuencia idéntica a la que F08-03 vino a resolver:** la cocina sigue preparando algo que ya se canceló y el pedido simplemente desaparece de su cola. Y son justo los dos caminos más naturales para gerencia, que cancela repasando la sala o el historial, no entrando a la venta. | `src/components/CancelOrderModal.tsx:13-22`, usado desde `src/pages/OrdersPage.tsx:5` y `src/components/OrderPreviewModal.tsx:77` | ✅ **Corregido y verificado el 20/08.** En vez de repetir la impresión en cada pantalla, la regla se sacó a un único sitio: **`src/domain/order.ts` → `barItemsForCancellation()`** decide qué renglones ya están en la barra (los `dispatched` y `prepared`, estampados con el motivo), y **`src/lib/orderCancellation.ts` → `cancelOrderNotifyingBar()`** hace las dos cosas juntas —cancelar y avisar— de modo que **cualquier punto de entrada, incluido uno que se añada mañana, hereda el aviso**. Los tres caminos existentes pasan ahora por ahí. El fallo de impresión se **devuelve** en vez de lanzarse: la cancelación ya quedó registrada y no debe deshacerse porque la impresora esté apagada, pero quien llama puede decírselo al usuario — y `CancelOrderModal`, que antes no tenía dónde, estrena aviso propio. **5 pruebas unitarias** nuevas sobre la regla pura. **Verificado en el navegador los tres caminos, uno por uno:** desde `/pedidos` (#1042, 2 renglones despachados) sale la comanda «CANCELACIÓN» con el motivo en cada renglón; desde la vista previa del **Salón** (#1043) también; y desde la propia venta (#1045) sale **exactamente una vez**, así que no hay duplicado ni regresión. Suite: **201 en verde** |
