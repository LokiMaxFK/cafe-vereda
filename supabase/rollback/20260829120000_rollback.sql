-- ROLLBACK de 20260829120000_cancelled_orders_and_recipe_inheritance.sql
--
-- LEE ESTO ANTES DE EJECUTAR
--
-- 1. Este script deshace el ESQUEMA, no los DATOS. Lo que la migración pudo escribir y sobrevive:
--    los movimientos 'daily_consumption' que la herencia de receta generó en ventas de productos con
--    presentación. Son consumo real y correcto — el fallo era no registrarlo — así que **no se
--    borran aquí**. Si de verdad los quieres fuera, míralos primero:
--      select * from public.inventory_movements
--       where movement_type = 'daily_consumption' and created_at >= '<fecha de aplicación>';
--
-- 2. Volver atrás reabre el agujero de cobro: una cuenta cancelada vuelve a poder cerrarse sin que
--    el servidor lance. Si haces rollback del SQL, vuelve también el cliente, porque las guardas de
--    `src/domain/order.ts` son lo único que quedaría tapando esa vía:
--      git checkout <commit anterior a esta tanda>
--
...
-- ---------------------------------------------------------------------------------------------
-- 1. prevent_immutable_changes vuelve a ignorar 'cancelled'  (versión de 20260813023800)
-- ---------------------------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------------------------
-- 2. sync_offline_operations vuelve a la versión de 20260824120000 (sin guarda de estado terminal)
-- ---------------------------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------------------------
-- 3. apply_sale_inventory_consumption vuelve a la versión de 20260826090000
--    (sin herencia de receta base y sin exigir estado de partida)
-- ---------------------------------------------------------------------------------------------
