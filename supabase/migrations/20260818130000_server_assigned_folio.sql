-- El folio impreso en la comanda no coincidía con el del historial.
--
-- El cliente calculaba un folio provisional (Math.max local + 1) y el servidor lo descartaba
-- para asignar el suyo desde order_folio_seq. La corrección llegaba unos segundos después, con
-- el pull remoto -- pero la comanda ya se había impreso con el número provisional, así que el
-- papel que llega a la barra y la orden en pantalla podían no coincidir.
--
-- Ahora el cliente reserva su folio de la misma secuencia al abrir la cuenta (next_order_folio)
-- y sync_offline_operations lo respeta. La secuencia sigue siendo la única autoridad, así que
-- dos dispositivos nunca reservan el mismo número.

create or replace function public.next_order_folio()
returns bigint language sql security invoker set search_path = '' as $$
  select nextval('public.order_folio_seq')
$$;

revoke execute on function public.next_order_folio() from public, anon;
grant execute on function public.next_order_folio() to authenticated;
