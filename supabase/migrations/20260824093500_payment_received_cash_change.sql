-- El ticket no podía imprimir el cambio porque el efectivo entregado no se guardaba en ningún
-- lado: `payments.amount_cents` se recorta al saldo de la cuenta (applyPaymentCap en el cliente),
-- así que un pago de $500 sobre una cuenta de $400 quedaba registrado como $400 y los $100 de
-- cambio no eran reconstruibles. Se agrega `received_cents` para conservar lo que entregó el
-- cliente; el cambio es la diferencia entre ambos.
--
-- Es nulo salvo en efectivo, y también en los pagos anteriores a esta migración: el ticket
-- simplemente no imprime el renglón de cambio cuando falta el dato, sin romper reimpresiones
-- de ventas viejas.
--
-- `amount_cents` no cambia de significado: sigue siendo lo que se queda en el cajón, que es lo
-- que suma el arqueo de caja. El cambio sale del mismo cajón, así que no debe contarse como venta.
--
-- Además se redefine sync_offline_operations para propagar el campo nuevo. Esa es la única
-- diferencia respecto a la versión anterior de la función
-- (20260818140000_incident_amount_includes_modifiers.sql): el resto del cuerpo se copia sin cambios.

alter table public.payments
  add column if not exists received_cents integer
  check (received_cents is null or received_cents >= amount_cents);

comment on column public.payments.received_cents is
  'Efectivo entregado por el cliente en un pago en efectivo. El cambio es received_cents - amount_cents. Nulo en tarjeta y transferencia.';
...
begin
  ...
  for v_operation in select * from jsonb_array_elements(p_operations) order by (value->>'createdAt')::timestamptz loop
    ...
    if v_type in ('create_order','add_order_item','update_order_item','dispatch_order_items','mark_order_ready','finalize_order','record_payment','apply_discount','cancel_dispatched_item','cancel_order','close_order') then
      ...
      insert into public.orders as o (id, folio, order_type, table_id, customer_name, status, opened_by, discount_cents, discount_reason, device_id, opened_at, updated_at, sync_status)
      ...
      for v_payment in select * from jsonb_array_elements(coalesce(v_payload->'payments','[]'::jsonb)) loop
        insert into public.payments(id, order_id, method, amount_cents, tip_cents, received_cents, recorded_by, idempotency_key, created_at)
