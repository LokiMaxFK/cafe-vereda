-- El importe registrado en las incidencias de cancelación (item_cancellation y order_cancellation)
-- se calculaba como precio_base × cantidad, ignorando los extras (modifiers) de la línea. Una
-- cuenta con Bebida vegetal ($15) sobre 2 Matcha ($90 c/u) vale $210, pero la incidencia quedaba
-- registrada como $180: el importe auditado no coincidía con lo que realmente se cobró o se iba a
-- cobrar. Afecta a las dos incidencias derivadas de order_items:
--   - item_cancellation: se corrige sumando el precio de los modifiers del artículo (v_modifiers,
--     ya construido antes en el mismo ciclo) antes de multiplicar por la cantidad.
--   - order_cancellation: se corrige sumando, por cada renglón no cancelado, el precio de sus
--     modifiers a su unit_price_cents antes de multiplicar por su cantidad.
-- sale_reversal no se toca: ya suma los pagos reales (payments.amount_cents), que sí incluyen los
-- extras porque el monto cobrado en el momento del pago ya los llevaba sumados.
--
-- Esta es la única diferencia respecto a la versión anterior de la función
-- (20260818130000_server_assigned_folio.sql): el resto del cuerpo se copia sin cambios.
